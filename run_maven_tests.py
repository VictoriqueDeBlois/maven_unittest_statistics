...
def get_env() -> dict:
    ...
    env['CLASSPATH'] = f".:{JAVA_HOME}/lib/dt.jar:{JAVA_HOME}/lib/tools.jar"
    env['PATH'] = f"{JAVA_HOME}/bin:{MAVEN_HOME}/bin:{env['PATH']}"
...
def append_log(log_file: Path, header: str, text: str):
...
# ── Git 操作 ──────────────────────────────────────────────────────────────────
def git_reset(project_path: Path, git_log_file: Path) -> bool:
    ...
    for cmd in [['git', 'reset', '--hard'], ['git', 'clean', '-fd']]:
        ...
        append_log(git_log_file, f"$ {' '.join(cmd)}", output)
...
# ── Maven target 目录 ─────────────────────────────────────────────────────────
def get_target_directory(project_path: Path) -> str:
    env = get_env()
    try:
        result = subprocess.run(
...
# ── Maven 测试 ────────────────────────────────────────────────────────────────
def run_maven_test(project_path: Path, target_path: str, mvn_log_file: Path) -> bool:
    """使用 JaCoCo agent 运行 mvn test，stdout/stderr 写入 mvn_log_file"""
    env = get_env()
    agent_jar = os.path.join(
        M2_HOME, 'repository', 'org', 'jacoco', 'org.jacoco.agent',
        JACOCO_VERSION,
...
# ── JaCoCo 报告 ───────────────────────────────────────────────────────────────
def get_modules(pom_file: str) -> list:
    try:
...
def report_jacoco_module(
    ...
    sub_modules = get_modules(pom_file)
    for sub in sub_modules:
        sub_rel = os.path.join(module, sub) if module else sub
        output_dirs.extend(report_jacoco_module(project_path, jacoco_exec, sub_rel, env))
    ...
    cmd = [
        'java', '-jar', JACOCO_CLI_JAR, 'report', jacoco_exec,
    ...
    try:
        ...
        logging.warning(f"jacoco report 失败 [{report_name}]: {e.stderr}")

    return output_dirs


def collect_jacoco_coverage(project_path: Path) -> dict:
    ...
    csv_files = report_jacoco_module(project_path, jacoco_exec, '', env)
    if not csv_files:
    ...
    for f in csv_files:
        try:
            ...
            logging.warning(f"读取 jacoco CSV 失败 [{f}]: {e}")
...
# ── CSV 写入 ──────────────────────────────────────────────────────────────────
def write_result(csv_path: Path, row: dict):
    with csv_lock:
...
def process_project(project_name: str, root_dir: Path, log_dir: Path, csv_path: Path):
    ...
    if not project_path.exists():
        ...
        logging.error(f"[{project_name}] 路径不存在")
        write_result(csv_path, result)
        return

    # Step 1: git reset
    logging.info(f"[{project_name}] git reset --hard && git clean -fd")
    if not git_reset(project_path, git_log_file):
        result['notes'] = 'git reset 失败'
        logging.error(f"[{project_name}] git reset 失败")
        write_result(csv_path, result)
        return

    # Step 2: 获取 target 目录
    target_path = get_target_directory(project_path)

    # Step 3: mvn test with jacoco agent
    logging.info(f"[{project_name}] 运行 mvn test")
    test_ok = run_maven_test(project_path, target_path, mvn_log_file)
    result['test_success'] = test_ok

    if not test_ok:
        result['notes'] = 'mvn test 失败'
        logging.warning(f"[{project_name}] mvn test 失败")
        write_result(csv_path, result)
        return

    # Step 4: 收集 JaCoCo 覆盖率
    logging.info(f"[{project_name}] 收集 JaCoCo 覆盖率")
    cov = collect_jacoco_coverage(project_path)
    result.update(cov)

    if not cov.get('jacoco_found'):
        logging.warning(f"[{project_name}] {cov.get('notes', 'jacoco 收集失败')}")
    else:
        logging.info(
    ...
    write_result(csv_path, result)
    logging.info(f"[{project_name}] 完成")
...
def main():
    ...
    logging.info(f"共 {len(projects)} 个项目，并行度 {args.parallel}")
    logging.info(f"结果将写入: {csv_path}")

    with ThreadPoolExecutor(max_workers=args.parallel) as executor:
        futures = {
            executor.submit(process_project, p, root_dir, log_dir, csv_path): p
            for p in projects
        }
        for future in as_completed(futures):
            ...
            try:
                ...
                logging.error(f"[{p}] 未捕获异常: {e}", exc_info=True)
